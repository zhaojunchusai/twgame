...
public class UIPanelInspector : UIRectEditor
{
    ...
    protected override bool ShouldDrawProperties()
    {
        ...
        GUILayout.BeginHorizontal();
        {
            EditorGUILayout.PrefixLabel("OffsetDepth");

            int offsetdepth = mPanel.offsetdepth;
            if (GUILayout.Button("Back", GUILayout.Width(60f))) --offsetdepth;
            offsetdepth = EditorGUILayout.IntField(offsetdepth, GUILayout.MinWidth(20f));
            if (GUILayout.Button("Forward", GUILayout.Width(68f))) ++offsetdepth;

            if (mPanel.offsetdepth != offsetdepth)
            {
                mPanel.offsetdepth = offsetdepth;
