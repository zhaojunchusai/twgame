...
public class UIScrollView : MonoBehaviour
{
    ...
    //public OnDragNotification onDragFinishedUp;
    public OnDragNotification onDragFinishedDown;
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			else
			{
                ...
                Vector3 mDragOver = mPanel.CalculateConstrainOffset(bounds.min, bounds.max);
                if (mDragOver.magnitude > 0.001f)
                {
                    //if (mDragOver.y > 0 && onDragFinishedUp != null)
                    //{
                    //    onDragFinishedUp();
                    //}

                    if (mDragOver.y < 0 && onDragFinishedDown != null)
                    {
                        onDragFinishedDown();
                    }
