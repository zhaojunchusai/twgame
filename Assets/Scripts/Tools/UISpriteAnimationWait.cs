...
public class UISpriteAnimationWait : MonoBehaviour
{
    [HideInInspector][SerializeField]protected int mFPS = 30;
    [HideInInspector][SerializeField]protected int mDepth = 1;
    [HideInInspector][SerializeField]protected float mWaitTime = 0;
    ...
    protected virtual void Update()
    {
        ...
        { 
            switch (mWaitState)
            {
               case 0:
               {
                    mCurrentTime += RealTime.deltaTime;
    ...
    private void PlayerAnimation() 
    {
        ...
        if (rate < mDelta)
        {
            ...
            if (++mIndex >= mSpriteNames.Count)
            {
                mIndex = 0;
                mActive = mLoop;
                if (mIsdisable && !mLoop && waitTime <=0)
                {
