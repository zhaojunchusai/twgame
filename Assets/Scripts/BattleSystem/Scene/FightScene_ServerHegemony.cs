...
/// 全服爭霸
/// </summary>
public class FightScene_ServerHegemony : FightSceneBase
{
    /// <summary>
    /// 路径数量
    /// </summary>
    ...
    public override bool Operate_Victory()
    {
        ...
        FightRelatedModule.Instance.SendServerHegemonyReq();
    ...
    private void CreateRole_Enemy()
    {
        ...
        //创建英雄//
        int tmpMaxHP = fightInfoEnemy.mAttribute.HP;
        int tmpCurHP = (int)(tmpMaxHP * SupermacyModule.Instance.HPValue);
        ...
        CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_RoleCreate,
            ...
            {
                roleObj.ReSetRoleHP(tmpCurHP - tmpMaxHP, HurtType.Crite, false, true);
                roleObj.pRpleHP.SetCurAndMaxValue(tmpCurHP, tmpMaxHP);
            ...
            for (int i = 0; i < fightInfoEnemy.mSoldierList.Count; i++)
            {
                ...
                for (int j = 0; j < fightInfoEnemy.mSoldierList[i].mNum; j++)
                {
                    if (fightInfoEnemy.mSoldierList[i].mSoldier == null)
                        continue;
                    tmpMaxHP = fightInfoEnemy.mSoldierList[i].mSoldier.showInfoSoldier.HP;
                    tmpCurHP = (int)(tmpMaxHP * SupermacyModule.Instance.HPValue);
                    CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_RoleCreate,
                        new CData_CreateRole(fightInfoEnemy.mSoldierList[i].mSoldier.showInfoSoldier, fightInfoEnemy.mSoldierList[i].mSoldier.uId,
                            Get_OtherIndex, ERoleType.ertSoldier, EHeroGender.ehgNone, EFightCamp.efcEnemy, sceneInfo.sceneType, 0, null, (roleObj) =>
                            {
                                roleObj.ReSetRoleHP(tmpCurHP - tmpMaxHP, HurtType.Crite, false, true);
                                roleObj.pRpleHP.SetCurAndMaxValue(tmpCurHP, tmpMaxHP);
                                base.CheckResourceIsReady(roleObj);
