...
/// 排位賽
/// </summary>
public class FightScene_Qualifying : FightSceneBase
{
    /// <summary>
    /// 路径数量
    /// </summary>
    ...
    public override bool Operate_Victory()
    {
        ...
        if (QualifyingModule.Instance.IsRevenge)
        {
            FightRelatedModule.Instance.SendEndPole(PoleResult.REVENGE_WIN, QualifyingModule.Instance.LogUID);
        }
        else
        {
            FightRelatedModule.Instance.SendEndPole(PoleResult.ATTACK_WIN, string.Empty);
        }
    ...
    public override bool Operate_Failure()
    {
        ...
        if (QualifyingModule.Instance.IsRevenge)
        {
            FightRelatedModule.Instance.SendEndPole(PoleResult.REVENGE_LOSE, QualifyingModule.Instance.LogUID);
        }
        else
        {
            FightRelatedModule.Instance.SendEndPole(PoleResult.ATTACK_LOSE, string.Empty);
        }
        CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_FightFinished, new FightFinishedInfo(_SceneStatus, 0));
        return true;
    }
    ...
    private void CreateRole_Self()
    {
        ...
        CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_RoleCreate,
            new CData_CreateRole(fightInfoSelf.mAttribute, 0, pathIndex_Hero, ERoleType.ertHero,
            (EHeroGender)fightInfoSelf.mGender, EFightCamp.efcSelf, sceneInfo.sceneType, 0, null, (roleObj) =>
            {
                ...
                for (int j = 0; j < fightInfoSelf.mSoldierList[i].mNum; j++)
                {
                    ...
                    CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_RoleCreate,
                        new CData_CreateRole(fightInfoSelf.mSoldierList[i].mSoldier.showInfoSoldier, fightInfoSelf.mSoldierList[i].mSoldier.uId,
                            Get_OtherIndex, ERoleType.ertSoldier, EHeroGender.ehgNone, EFightCamp.efcSelf, sceneInfo.sceneType, 0, null, CheckResourceIsReady));
                }
    ...
    private void CreateRole_Enemy()
    {
        ...
        CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_RoleCreate,
            new CData_CreateRole(fightInfoEnemy.mAttribute, 0, pathIndex_Hero, ERoleType.ertHero,
            (EHeroGender)fightInfoEnemy.mGender, EFightCamp.efcEnemy, sceneInfo.sceneType, 0, null, (roleObj) =>
            {
                ...
                for (int j = 0; j < fightInfoEnemy.mSoldierList[i].mNum; j++)
                {
                    ...
                    CommandManager.Instance.SendSingleCommand(MessageID.Message_Fight.FM_RoleCreate,
                        new CData_CreateRole(fightInfoEnemy.mSoldierList[i].mSoldier.showInfoSoldier, fightInfoEnemy.mSoldierList[i].mSoldier.uId,
                            Get_OtherIndex, ERoleType.ertSoldier, EHeroGender.ehgNone, EFightCamp.efcEnemy, sceneInfo.sceneType, 0, null, CheckResourceIsReady));
                }
